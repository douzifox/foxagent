...
前置：[Bun](https://bun.sh)、[ripgrep](https://github.com/BurntSushi/ripgrep)（`search` 工具靠它）、
...
下文命令都按 clone 到 `~/foxagent` 写，放别处自行替换路径。
...
```bash
cd 你的项目
bun ~/foxagent/src/cli.ts             # 默认继续该项目最近一次会话
bun ~/foxagent/src/cli.ts --new       # 开新会话
